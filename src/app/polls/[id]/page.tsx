...
export default function CreatePollPage() {
  ...
  const poolNum = parseInt(challengePool) || 0
  const estimatedParticipants = [100, 500, 1000]
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isChallenge && (poolNum < 100 || poolNum > 1_000_000)) {
      setError('Token pool must be between 100 and 1,000,000.')
      setBusy(false)
      return
    }
    ...
    const { data: pollId, error: rpcError } = await supabase.rpc('create_poll', {
      ...
      p_is_hot_take: canHotTake && isHotTake && !isCommunity && !isChallenge,
      p_is_community: isCommunity && !isChallenge,
      p_community_name: isCommunity && !isChallenge ? communityName.trim() || null : null,
      p_community_code: isCommunity && !isChallenge ? communityCode : null,
      p_community_password: isCommunity && !isChallenge ? communityPassword.trim() || null : null,
      p_is_challenge: isChallenge,
      p_challenge_pool: isChallenge ? poolNum : 0,
    })
  ...
  return (
    <main className="max-w-xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 sm:gap-6">
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              className={`flex items-center justify-between gap-3 rounded-xl border-2 p-4 text-left transition-all ${
                isChallenge ? 'border-amber-500 bg-amber-50' : 'border-border hover:border-amber-400/60'
              }`}
            >
              <div>
                <p className="text-sm font-bold text-foreground flex items-center gap-1.5">
                  <Trophy size={15} className={isChallenge ? 'text-amber-600' : ''} /> Challenge 🏆
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">Token prize pool shared among all participants</p>
              </div>
            ...
            {isChallenge && (
              <div className="flex flex-col gap-3 pl-1">
                <div>
                  <label className="block text-sm font-semibold text-foreground mb-1.5">Token Pool</label>
                  <input
                    type="number"
                    value={challengePool}
                    ...
                    min={100}
                    max={1000000}
                    className="w-full border border-border rounded-xl px-4 py-3 text-base bg-transparent outline-none focus:ring-2 focus:ring-amber-400 placeholder:text-muted-foreground min-h-[44px]"
                  />
                  <p className="text-xs text-muted-foreground mt-1">Tokens shared equally among all participants</p>
                </div>
                {poolNum >= 100 && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex flex-col gap-1.5">
                    <p className="text-xs font-bold text-amber-800">Estimated rewards</p>
                    {estimatedParticipants.map((n) => (
                      <p key={n} className="text-xs text-amber-700">
                        With {n.toLocaleString()} participants: ~{Math.floor(poolNum / n).toLocaleString()} tokens each
                      </p>
                    ))}
                  </div>
                )}
        ...
        {/* Community poll toggle — hidden when challenge is on */}
        {!isChallenge && <div className="flex flex-col gap-3">
          <button
            type="button"
