// Nigeria's 774 Local Government Areas (LGAs), keyed by state name.
// State keys match exactly the names in `./states.ts` (NIGERIAN_STATES),
// so the voting UI can look up a state's LGAs by the selected state value.
...
// Flat list of every LGA name (rarely needed, but handy for validation).
export const ALL_LGAS: string[] = Object.values(LGAS_BY_STATE).flat()
